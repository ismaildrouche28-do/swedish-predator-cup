  ...
  return (
    <div>
      ...
      <section className="bg-white rounded-3xl overflow-hidden shadow-cs-sm mb-3">
        <div className="px-3 sm:px-5 pt-4 pb-2 grid grid-cols-[30px_minmax(0,1fr)_44px_38px_62px_42px] lg:grid-cols-[42px_1fr_74px_54px_100px_50px] gap-2 sm:gap-3 text-[9px] sm:text-[10px] uppercase tracking-wide sm:tracking-widest font-bold text-ink-3">
          <span>#</span><span>Name</span><span className="text-right whitespace-nowrap">Pkt</span><span className="text-right whitespace-nowrap">Fänge</span><span className="text-right whitespace-nowrap">Bonus</span><span className="text-right whitespace-nowrap">Trend</span>
        </div>
        ...
        {ranking.map((r: any, idx: number) => {
          ...
          return (
            <Link key={r.user_id} href={`/spieler/${r.user_id}`}
              className={`grid grid-cols-[30px_minmax(0,1fr)_44px_38px_62px_42px] lg:grid-cols-[42px_1fr_74px_54px_100px_50px] gap-2 sm:gap-3 items-center px-3 sm:px-5 py-3 border-t border-black/[0.04] hover:bg-spc-greyLight transition ${r.user_id === user.id ? "bg-spc-lighter/60" : ""}`}>
              <Medal rank={idx + 1} />
              <div className="min-w-0">
